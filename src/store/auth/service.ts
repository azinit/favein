import API from "api"
import { setToken, setUser, addFave as addFaveStore, deleteFave as deleteFaveStore, resetAuthPayload, addError } from "./slice"

export const signIn = () => async (dispatch: any, getState: GlobalStateGetter) => {
    const { email, password } = getState().auth.authPayload
    if (email && password) {
        const responseLogin = await API.users.signIn(email, password).catch(e => {
            if (e.response.status === 403) {
                dispatch(addError("Неверные email или парль"))
            }
        })
